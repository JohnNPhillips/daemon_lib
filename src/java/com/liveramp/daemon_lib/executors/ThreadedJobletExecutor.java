...
public class ThreadedJobletExecutor<T extends JobletConfig> implements JobletExecutor<T> {
  ...
  public ThreadedJobletExecutor(ThreadPoolExecutor threadPool, JobletFactory<T> jobletFactory, JobletCallback<T> successCallback, JobletCallback<T> failureCallback) {
    this(threadPool, jobletFactory, successCallback, failureCallback, new ErrorCallback.None<T>());
  }

  public ThreadedJobletExecutor(ThreadPoolExecutor threadPool, JobletFactory<T> jobletFactory, JobletCallback<T> successCallback, JobletCallback<T> failureCallback, ErrorCallback<T> errorCallback) {
  ...
  public void execute(final T config) throws DaemonException {
    threadPool.submit(new Callable<Void>() {
      ...
      public Void call() throws Exception {
        ...
        } catch (JobletException je) {
          LOG.error("Caught joblet exception with config " + config, je);
          errorCallback.callback(config, je.getVal(), je.getMessage());
        } catch (Exception e) {
